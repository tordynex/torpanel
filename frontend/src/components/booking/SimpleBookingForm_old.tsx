...
const SimpleBookingForm: React.FC<Props> = ({
  ...
  const setAsPrimary = async (customerId: number) => {
    ...
    setSuccessNote("");
    try {
      // Hämta kund och använd "createCustomer" för att dedupe + sätta primär-länk
    ...
    try {
      ...
      for (let i = 0; i < totalParts; i++) {
        ...
        const res = await autoScheduleBooking({
          ...
          end_at: part.end_at,
          assigned_user_id: selectedProposal.assigned_user_id ?? undefined,
          title: `${title || "Bokning"}${
    ...
    <div className={`${styles.shell} ${className ?? ""}`}>
          ...
          <section className={styles.card}>
            ...
            <div className={styles.field}>
              <div className={styles.segment}>
                <button
                  ...
                  className={`${styles.segmentBtn} ${
                    dateMode === "latest" ? styles.segmentBtnActive : ""
                  }`}
                  onClick={() => {
                    setDateMode("latest");
                ...
                <button
                  type="button"
                  className={`${styles.segmentBtn} ${
                    dateMode === "range" ? styles.segmentBtnActive : ""
                  }`}
              ...
              {dateMode === "range" && (
                <div className={styles.inlineGrid}>
                  <div className={styles.field}>
                    <label htmlFor="earliest" className={styles.label}>
                      Tidigt från
                    </label>
                    <div className={styles.inputWrap}>
                      <span className={styles.inputIcon}>
                      ...
                      <input
                        ...
                        type="datetime-local"
                        value={earliestFrom}
                        onChange={(e) => setEarliestFrom(e.target.value)}
                      />
                    </div>
                  </div>
      ...
      {!!proposals.length && (
        <section className={styles.card}>
          ...
          <div role="radiogroup" aria-label="Välj förslag">
            <table className={styles.table}>
              ...
              <tbody>
                {proposals.map((p, i) => (
                  <tr
                    key={`${p.bay_id}-${p.assigned_user_id ?? "na"}-${p.start_at}-${p.end_at}-${i}`}
                    className={selectedIndex === i ? styles.rowActive : undefined}
                  ...
                  >
                    ...
                    <td>{fmt(p.start_at)}</td>
                    <td>{fmt(p.end_at)}</td>
                    <td>{p.notes ?? `Bay #${p.bay_id}`}</td>
                    <td>
                      {p.assigned_user_id
                        ? employees.find((e) => e.id === p.assigned_user_id)?.username ||
                          `User #${p.assigned_user_id}`
                        : "Ej satt"}
                    </td>
                  </tr>
                ))}
          ...
          <div className={styles.inlineGrid}>
            <div className={styles.field}>
              <label htmlFor="bufb" className={styles.label}>
                Buffer före (min)
              </label>
              <div className={styles.inputWrap}>
                <span className={styles.inputIcon}>
                ...
                <input
                  ...
                  value={bufferBefore}
                  onChange={(e) =>
                    setBufferBefore(Number(e.target.value || 0))
                  }
                />
              </div>
            </div>
